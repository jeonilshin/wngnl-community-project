...
router.use(async (req, res, next) => {
    if(req.session && req.session.user){ res.cookie("user", req.session.user, { maxAge: 1000 * 60 * 60 * 24 }) }
    next();
})

router.get('/', async (req, res) => { res.sendFile("intro.html", { root: "public/intro/" }) })
...
router.get('/board', async (req, res) => {
    try{
        ...
        res.json({ status: 200, boardData: s3Response })
    }
    catch(error){
        console.log(error)
